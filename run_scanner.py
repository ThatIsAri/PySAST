...
try:
    ...
    print(f"Не удалось импортировать из pysast: {e}")

    #Пряма загрузка модулей
    try:
        #Иначе из директории корня
        import scanner
        ...
        for root, dirs, files in os.walk(current_dir):
            ...
            subindent = ' ' * 2 * (level + 1)
            for file in files[:5]:  #Показать 5 файлов
                if file.endswith('.py'):
...
Примеры использования:
  ...
  %(prog)s list-patterns               # Показать все шаблоны уязвимостей
    ...
    if args.command == 'scan':
        ...
        #Если команда не указана - вывести справку
        parser.print_help()
        sys.exit(0)
    else:
...
def run_scan(args):
    ...
    #Вывести информацию о сканировании
    if not args.quiet:
        print("=" * 60)
    ...
    try:
        ...
        if args.format == 'console' and not args.quiet:
            try:
                ...
                print(f"Модуль report_generator не найден: {e}")
        ...
        if critical_count > 0:
            ...
            if not args.quiet:
                print(f"\nВНИМАНИЕ: Найдено {high_count} высокоуровневых уязвимостей!")
...
def run_gitlab_scan(args):
    ...
    #Проверка запуска в среде gitlab CI
    ci_env = os.getenv('CI', 'false')

    #Получение аттрибута
    if not getattr(args, 'quiet', False) and ci_env != 'true':
        print("Предупреждение: Не обнаружена среда GitLab CI")
    ...
    #Создать экземпдяры классов
    scanner = PySASTScanner()
    gitlab_integration = GitLabIntegration(scanner)

    #Вывести, когда не тихий режим
